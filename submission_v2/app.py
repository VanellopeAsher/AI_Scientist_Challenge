        ...
        async def generate():
            ...
            try:
                ...
                def progress_callback(step, message):
                    """将进度信息放入队列"""
                    try:
                        progress_queue.put(("progress", step, message), timeout=1.0)
                    except queue.Full:
                        pass  # 忽略队列满的情况
                    except Exception as e:
                        # 静默处理错误，确保不影响主流程
                ...
                # 创建工作流（传入进度回调）
                workflow = IntegratedWorkflow(config, progress_callback=progress_callback)
                ...
                # 启动工作流任务
                workflow_task = loop.run_in_executor(None, run_workflow)
                ...
                while True:
                    ...
                    if workflow_task.done():
                        ...
                        while not progress_queue.empty():
                            try:
                                msg_type, step, message = progress_queue.get_nowait()
                ...
                if isinstance(result, tuple) and len(result) >= 4:
                    result_dict, plan, review, references = result[0], result[1], result[2], result[3]
                elif isinstance(result, tuple) and len(result) >= 3:
                    result_dict, plan, review = result[0], result[1], result[2]
                    references = result_dict.get('references', '') if isinstance(result_dict, dict) else ''
                else:
                    # 如果返回格式不同，尝试提取review和references
                ...
                if references:
                    # 添加分隔符
                    separator = "\n\n## References\n\n"
                    for i in range(0, len(separator), chunk_size):
                        chunk = separator[i:i + chunk_size]
                        response_data = {
                            "object": "chat.completion.chunk",
                            "choices": [{
                                "delta": {
                    ...
                    # 流式输出references内容
                    references_text = references if isinstance(references, str) else str(references)
                    for i in range(0, len(references_text), chunk_size):
                        chunk = references_text[i:i + chunk_size]
                        response_data = {
                            "object": "chat.completion.chunk",
                            "choices": [{
                                "delta": {
