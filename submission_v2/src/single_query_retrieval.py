...
class SingleQueryRetrieval:
    ...
    def search_arxiv(self, query, max_results=None):
        ...
        # 多获取一些，确保有足够论文（至少是请求数量的2倍）
        # 根据新需求，初始检索150篇
        search_max = max(max_results * 2, 150)  # 至少检索150篇
    ...
    def rerank_with_local_ranker(self, outline_paragraph, candidate_papers, top_k=40):
        """
        使用本地重排序器（4因子评分系统）对候选论文进行重排序
        
        Args:
            outline_paragraph: 研究提纲段落（用于评分）
        ...
        print(f"正在使用4因子评分系统对 {len(candidate_papers)} 篇论文进行重排序...")
        print(f"目标：选择前 {top_k} 篇论文")
        
        # 创建本地重排序器
        local_ranker = LocalRanker(self.config)
        
        # 如果没有提供outline_paragraph，使用topic_description作为替代
        if not outline_paragraph:
            outline_paragraph = "General research topic"  # 默认值
        
        # 执行重排序
        reranked_papers = local_ranker.rerank_papers_simple(
    ...
    def retrieve(self, topic_description, use_local_ranker=True, outline_paragraph=None, top_k=40):
        """
        执行完整的检索流程
        
        Args:
            topic_description: 主题描述
            use_local_ranker: 是否使用本地4因子重排序器（默认True）
            outline_paragraph: 研究提纲段落（用于评分，如果为None则使用topic_description）
            top_k: 返回前k篇论文（默认40）
        """
        print("=" * 60)
        ...
        # 步骤2: 在arxiv上搜索（检索150篇）
        candidate_papers = self.search_arxiv(arxiv_query, max_results=150)
        ...
        # 限制为150篇
        if len(candidate_papers) > 150:
            candidate_papers = candidate_papers[:150]
            print(f"限制候选论文数量为150篇")
        
        print(f"检索到 {len(candidate_papers)} 篇候选论文")
        
        # 步骤3: 使用重排序
        if use_local_ranker:
            # 使用新的4因子评分系统
            if outline_paragraph is None:
                outline_paragraph = topic_description
            reranked_papers = self.rerank_with_local_ranker(
                outline_paragraph, 
                candidate_papers, 
                top_k=top_k
            )
